player_standing_1:
	file "../res/player/st1.pmb"
	

player_standing_2:
	file "../res/player/st2.pmb"
	

player_standing_3:
	file "../res/player/st3.pmb"
	

player_running_left_1:
	file "../res/player/rl1.pmb"
	

player_running_left_2:
	file "../res/player/rl2.pmb"
	

player_running_left_3:
	file "../res/player/rl3.pmb"
	

player_running_left_4:
	file "../res/player/rl4.pmb"
	

player_running_right_1:
	file "../res/player/rr1.pmb"
	

player_running_right_2:
	file "../res/player/rr2.pmb"
	

player_running_right_3:
	file "../res/player/rr3.pmb"
	

player_running_right_4:
	file "../res/player/rr4.pmb"
	

player_running_up_1:
	file "../res/player/ru1.pmb"
	

player_running_up_2:
	file "../res/player/ru2.pmb"
	

player_running_down_1:
	file "../res/player/rd1.pmb"
	

player_running_down_2:
	file "../res/player/rd2.pmb"
	
